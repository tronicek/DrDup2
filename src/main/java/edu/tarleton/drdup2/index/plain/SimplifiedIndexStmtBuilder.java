 ...
 * index. It uses the blind renaming strategy.
 *
 * @author Zdenek Tronicek, tronicek@tarleton.edu
 */
public class SimplifiedIndexStmtBuilder extends IndexBuilder {
    ...
    public SimplifiedIndexStmtBuilder(Properties conf, Path srcDir) {
        super(conf, srcDir);
    }
    ...
    private void appendWithPos(String type) {
        ...
        for (StackNode node : stack) {
            Pos pos = node.getPos();
            TrieNode tnode = node.getNode();
            TrieNode p = tnode.addChild(type, pos);
            s.push(p, pos);
        }
        stack = s;
    }
    ...
    private void appendId() {
        ...
        for (StackNode node : stack) {
            Pos pos = node.getPos();
            TrieNode tnode = node.getNode();
            TrieNode p = tnode.addChild("id", pos);
            s.push(p, pos);
        }
        stack = s;
    }
